...
from streamdal import Audience, StreamdalConfig, StreamdalClient, ProcessRequest, ProcessResponse, \
    OPERATION_TYPE_CONSUMER, OPERATION_TYPE_PRODUCER, EXEC_STATUS_ERROR
...
def streamdal_process(streamdal_client: StreamdalClient, operation_type: int, exchange_name: str, routing_key: str, msg: bytes, cfg: StreamdalRuntimeConfig) -> bytes:
    ...
    if resp.status == EXEC_STATUS_ERROR:
        ...
        if len(cfg) > 0 and cfg[0].strict_errors:
